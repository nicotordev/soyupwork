...
export async function StudentCertificatesView() {
  ...
  return (
    <DashboardContainer>
      ...
      <div className={cn(adminPanelClass, "p-6 sm:p-8 bg-card shadow-[6px_6px_0px_0px_var(--foreground)]")}>
        {certificates.length === 0 ? (
          <div className="flex flex-col items-center gap-6 py-12 text-center max-w-lg mx-auto">
            ...
            {/* Motivational steps */}
            <div className="w-full text-left border-2 border-foreground bg-muted/20 p-4 rounded-lg space-y-3 font-mono text-[11px] font-bold uppercase tracking-wide">
              <div className="flex items-center gap-2">
                <span className="flex size-5 shrink-0 items-center justify-center rounded-full border border-foreground bg-secondary">1</span>
                <span>Inscríbete en un curso con certificado</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="flex size-5 shrink-0 items-center justify-center rounded-full border border-foreground bg-secondary">2</span>
                <span>Completa todas las lecciones del temario</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="flex size-5 shrink-0 items-center justify-center rounded-full border border-foreground bg-emerald-500/20 text-emerald-700 border-emerald-500/30">3</span>
          ...
          <div className="space-y-4">
            ...
            <ul className="grid gap-4 md:grid-cols-2">
              {certificates.map((cert) => (
                <li
                  key={cert.id}
                  className="flex flex-col justify-between gap-4 border-2 border-foreground bg-card p-5 rounded-lg shadow-[4px_4px_0px_0px_var(--foreground)] transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_0px_var(--foreground)]"
                >
                  <div className="flex items-start gap-4 text-left">
                    ...
                    <div className="min-w-0 space-y-1">
                      ...
                      <p className="font-mono text-[10px] font-semibold text-emerald-600 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-sm w-fit leading-none mt-2">
                        Completado
                      </p>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between border-t border-foreground/15 pt-3 mt-1">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className={cn(adminBrutalButtonClass, "bg-background py-1 px-3 h-8")}
                    >
                      <Link

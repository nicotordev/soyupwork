...
type CourseLandingSyllabusProps = {
  ...
  syllabusPlan: readonly SyllabusPlanItem[];
  ...
  return (
    ...
    >
      ...
      <Accordion
        type="single"
        collapsible
        className="space-y-3 border-0 bg-transparent"
      >
        {syllabusPlan.map((module, idx) => (
          <AccordionItem
            key={module.title}
            value={`module-${idx}`}
            className="rounded-2xl border border-foreground/10 bg-card px-2"
          >
            <AccordionTrigger className="px-3 py-4 no-underline hover:no-underline">
            ...
            <AccordionContent className="px-3 pb-4">
              <div className="space-y-2">
                <div className="rounded-xl border border-foreground/10 bg-background p-3 text-xs">
                  Incluye: plantillas, checklist operativa, proyecto aplicado y
                  quiz.
                </div>
              </div>
            </AccordionContent>
      ...
      {hasAnyLessons ? (
        <div className="space-y-4">
          <h3 className="text-lg font-bold">Vista de lecciones reales</h3>
          <div className="space-y-4">
              ...
              >
                ...
                <ul className="divide-y divide-foreground/10">
                  {module.lessons.map((lesson, lessonIndex) => {
                    ...
                    const completed =
                      view.hasFullAccess && lessonIndex % 4 === 0;
                    const row = (

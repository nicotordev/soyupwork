...
export function getDummyQuizPlayData(lessonId: string): GetQuizPlayDataResult {
  const { quiz: quizLessonId } = getDummyDemoLessonIds();
  if (lessonId !== quizLessonId) {
...
export function submitDummyQuizAttempt(
  ...
  const totalQuestions = DUMMY_QUIZ.questions.length;
  ...
  for (const question of DUMMY_QUIZ.questions) {
  ...
  const passed = score >= DUMMY_QUIZ.passingScore;
